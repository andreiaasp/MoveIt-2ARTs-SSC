...
class PoseClassifierProcessor @WorkerThread constructor(context: Context, isStreamMode: Boolean) {
    ...
    private fun loadPoseSamples(context: Context) {
        ...
        try {
            val reader = BufferedReader(
                InputStreamReader(context.assets.open(PoseClassifierProcessor.Companion.POSE_SAMPLES_FILE))
            )
            var csvLine = reader.readLine()
            while (csvLine != null) {
                // If line is not a valid {@link PoseSample}, we'll get null and skip adding to the list.
                val poseSample: PoseSample = PoseSample.Companion.getPoseSample(csvLine, ",")!!
                poseSamples.add(poseSample)
                csvLine = reader.readLine()
            }
        } catch (e: IOException) {
            Log.e(PoseClassifierProcessor.Companion.TAG, "Error when loading pose samples.\n$e")
        }
        poseClassifier = PoseClassifier(poseSamples)
        if (isStreamMode) {
            for (className in PoseClassifierProcessor.Companion.POSE_CLASSES) {
    ...
    companion object {
        ...
        private val POSE_CLASSES = arrayOf<String>(
            PoseClassifierProcessor.Companion.PUSHUPS_CLASS,
            PoseClassifierProcessor.Companion.SQUATS_CLASS
        )
    }
